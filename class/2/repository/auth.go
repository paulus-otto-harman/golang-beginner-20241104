...
func (repo *Auth) Authenticate(user *model.User) (*model.Session, error) {
	query := `INSERT INTO sessions (user_id) SELECT id FROM users WHERE email=$1 AND password=$2 RETURNING id`
	var session model.Session

	if err := repo.Db.QueryRow(query, user.Email, user.Password).Scan(&session.Id); err != nil {
		return nil, err
	}
	return &session, nil
}
